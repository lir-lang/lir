...
namespace lir {
	...
	// in_group<'a', 'b', 'c'>('a')
	template <char... Cs>
	constexpr __attribute__((const)) bool in_group(const char c) noexcept { return ((c == Cs) or ...); }
	...
	constexpr __attribute__((const)) bool alpha(const char c) noexcept {
		return lower(c) or upper(c);
	};


	constexpr __attribute__((const)) bool alphanumeric(const char c) noexcept {
	...
	constexpr __attribute__((const)) char lowest(const char a, const char b, Ts&&... args) noexcept {
		return ((a < b) ? a : b) < lowest(std::forward<Ts>(args)...);
	}


	template <typename... Ts>
	constexpr __attribute__((const)) char highest(const char a, const char b, Ts&&... args) noexcept {
